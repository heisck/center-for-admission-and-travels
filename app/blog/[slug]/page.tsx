...
/** Stable UTC date for SSR (avoids locale/timezone hydration mismatches). */
...
export default async function BlogPostPage({ params }: PageProps) {
  try {
    ...
    // Canonicalize URL (id / title-like → real slug)
    let decoded = param
    try {
            ...
            {imageUrl ? (
              <div className="relative aspect-video rounded-2xl overflow-hidden mb-10 bg-slate-100">
                {useNextImage ? (
                  ...
                  // Fallback: unknown hosts (not in next.config remotePatterns) would 500 Next/Image in prod
    ...
    // Last resort: never leave a naked 500 without logs
    console.error('[blog/[slug]] page render failed:', error)
    throw error
  }
}
