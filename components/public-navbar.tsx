      ...
      <nav className="fixed inset-x-0 top-0 z-[100] border-b border-black/5 bg-white/90 shadow-sm backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center gap-3 h-16 min-[1920px]:h-20 min-[2560px]:h-24">
            ...
            <div className="hidden xl:flex flex-1 items-center justify-center gap-0.5 min-w-0 px-2">
              {headerNavLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  title={link.label}
                  className={`px-2 py-2 transition text-sm font-medium whitespace-nowrap shrink-0 ${
